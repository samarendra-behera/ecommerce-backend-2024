...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const [thisMonthProducts, thisMonthUsers, thisMonthOrders, lastMonthProducts, lastMonthUsers, lastMonthOrders, lastSixMonthOrders, productsCount, usersCount, femaleUserCount, allOrders, productCategoriesCount, latestOrders] = await Promise.all([
        ...
        const lastMonthRevenue = lastMonthOrders.reduce((total, order) => { return total + (order.total || 0); }, 0);
        const changePercent = {
...
export const getPieCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const [orderFulfillmentRatio, productCategoriesCount, productsCount, productOutStock, allOrders, allUsers, customerCounts, adminCounts] = await Promise.all([
...
export const getBarCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const [sixMonthUsers, sixMonthProducts, twelveMonthOrders] = await Promise.all([
...
export const getLineCharts = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const [twelveMonthUsers, twelveMonthProducts, twelveMonthOrders] = await Promise.all([
